...
def main(
    ...
    """
    colmap_path = Path(__file__).parent / colmap_path
    ...
    print(f'Камер найдено: {len(cameras)}')
    print(f'Кадров найдено: {len(images)}')
    print(f'Точек найдено: {len(points3d)}')
    ...
    def visualize_frames() -> None:
        ...
        for img_id in tqdm(img_ids):
            ...
            print(f"Ищу файл: {image_filename}")
            if not image_filename.exists():
                print(f"Файл {image_filename} НЕ найден!")
